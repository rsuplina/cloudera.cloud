...
description:
    - Gathers information about single CDP Public IAM role or all roles
author:
...
- name: Gather information about a specific role
  cloudera.cloud.iam_role_info:
    name: crn:altus:iam:us-east-1:cm:role:ClassicClustersCreator

- name: Gather information about several roles
  cloudera.cloud.iam_role_info:
    name:
      - crn:altus:iam:us-east-1:cm:role:ClassicClustersCreator
      - crn:altus:iam:us-east-1:cm:role:DFCatalogAdmin
"""

RETURN = r"""
roles:
  description: Retrieve details about all selected IAM roles
  type: list
  returned: always
  elements: dict
  contains:
...
class IAMRoleInfo(CdpModule):
    ...
    def process(self):
        self.info = self.cdpy.iam.list_roles(self.name)
